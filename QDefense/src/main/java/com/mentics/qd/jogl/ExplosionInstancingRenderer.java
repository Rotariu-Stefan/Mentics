...
public class ExplosionInstancingRenderer implements ItemRenderer<Explosion> {
    ...
    public void initialize(GL2 gl2) {
        ...
        final ShaderCode vertShader =
                ShaderCode.create(gl, GL2ES2.GL_VERTEX_SHADER, Sphere.class, "", "", "explosion_i", true);
        final ShaderCode fragShader =
                ShaderCode.create(gl, GL2ES2.GL_FRAGMENT_SHADER, Sphere.class, "", "", "explosion_i", true);
    ...
    @Override
    public void render(GL2 gl2, List<Explosion> explosions) {
        final GL3ES3 gl = (GL3ES3)gl2;
        final int instanceCount = explosions.size();

        fbCenters.rewind();
        for (final Explosion explosion : explosions) {
            for (final float f : explosion.position) {
                fbCenters.put(f);
            }
        }
        fbCenters.rewind();
